...
def setup_metapredict(force):
    ...
    if installed(python, "metapredict") and not force:
        ...
        try:
            if os.name == "nt":
                # The published sdist is known to contain Windows-illegal
                # fixture names, so do not spend time attempting pip first.
                raise SystemExit("Windows requires the filtered source install")
            run([python, "-m", "pip", "install", "-q",
                 f"metapredict=={METAPREDICT_VERSION}"])
            print("  metapredict installed from the published distribution")
        except SystemExit:
            print("  installing a Windows-safe patched sdist")
            # Keep disposable build sources beside the disposable environments.
            # A OneDrive-synced vendor tree can retain locked __pycache__ files
            # and make an otherwise idempotent setup impossible to clean.
            source = fetch_metapredict_source(
                os.path.join(ENVS, "_sources", "metapredict_src")
            )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "rebuild only the two disposable Python environments; keep an "
            "existing PSpred checkout and its model files"
        ),
    )
    ...
    force_envs = args.force or args.force_envs
    metapredict_python = setup_metapredict(force_envs)
    pslab_python = setup_pslab(force_envs)
    ...
    print("\nsetup complete. Next:")
    print("    python build_dataset.py --help")
